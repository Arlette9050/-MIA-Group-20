...
def combining_transforms():
    ...
    X = util.test_object(1)
    X_t = reg.reflect(-1,1).dot(X)
    fig = plt.figure(figsize=(5,5))
    ax1 = fig.add_subplot(111)
    util.plot_object(ax1, X)
    util.plot_object(ax1, X_t)
    ax1.grid()
#.dot(reg.rotate(np.pi/2))
...
def arbitrary_rotation():
    ...
    t = X[:,0]
    phi = 0.25*np.pi
    Tarb = util.t2h(np.identity(2), t)

    list = [np.cos(phi),-np.sin(phi),0,np.sin(phi),np.cos(phi),0,0,0,1]
    Tang = np.reshape(np.array(list),[3,3])

    Tarbmi = util.t2h(np.identity(2), -t)
 
    T = Tarb.dot(Tang).dot(Tarbmi) 
    X_rot = T.dot(Xh)
